...
- Uvicorn worker processes requested: **8**

## Cold start

- process spawn -> `/readyz` 200: **3850 ms** (includes interpreter boot, sklearn import, and `joblib.load`)
- `/healthz` -> 200 in 21.6 ms
- `/readyz`  -> 200 in 10.9 ms  `{"status":"ready","model_version":"20260908T061824666806Z-4888d94c"}`
...
Closed-loop, 8s per level, 8 uvicorn worker(s), 16 logical CPUs on this machine. Latency in milliseconds.

| concurrency | requests | errors | RPS | mean | p50 | p95 | p99 | max |
|---|---|---|---|---|---|---|---|---|
| 1 | 222 | 0 | **27.7** | 36.0 | 34.9 | **49.5** | 53.6 | 344.3 |
| 4 | 732 | 0 | **89.8** | 44.1 | 37.3 | **84.1** | 195.8 | 479.4 |
| 16 | 1440 | 0 | **166.8** | 90.6 | 35.6 | **273.6** | 1390.5 | 3379.2 |
| 64 | 1553 | 0 | **134.4** | 385.2 | 39.9 | **2983.8** | 5425.9 | 5642.1 |

- Peak throughput observed: **166.8 req/s** at concurrency 16, p95 **273.6 ms**.
...
- SQLite path: `C:\Users\adars\Desktop\One ML\student-mental-health-data-analysis\runtime\serving-audit-workers-8.sqlite3`
- Successful prediction responses in this audit: **3951**
- New SQLite records captured in this audit: **3931**
- Record count did not match successful responses; investigate before treating this as an audit trail.

## Server stderr evidence

- Server stderr path: `C:\Users\adars\Desktop\One ML\student-mental-health-data-analysis\runtime\serving-audit-workers-8.stderr.log`
- Prediction-log write failures recorded by the server: **20**
